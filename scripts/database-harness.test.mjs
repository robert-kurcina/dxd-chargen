...
test('SQLite migrations, adapter, constraints, rollback and consistent backup restore', async () => {
  ...
  try {
    ...
    migrateDatabase(connection, migrationsFolder); assert.equal(count(), 4);
    ...
    assert.equal(restored.sqlite.prepare('SELECT count(*) AS n FROM __drizzle_migrations').get().n, 4);

...
export default function PortraitStep({ draft, setDraft }: { draft: CharacterDraft; setDraft: Dispatch<SetStateAction<CharacterDraft>> }) {
  ...
  return <div className="space-y-4">
    <section className="grid gap-3 rounded-lg border p-4 md:grid-cols-2"><div className="space-y-2"><label className="text-sm font-medium">Upload image</label><Input type="file" accept="image/*" onChange={loadFile} /></div><div className="space-y-2"><label className="text-sm font-medium">Image URL</label><div className="flex gap-2"><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://…" /><Button type="button" variant="outline" onClick={() => { const portraitSourceDataUrl = url.trim(); setSource(portraitSourceDataUrl); setDraft((current) => ({ ...current, utilities: { ...current.utilities, portraitSourceDataUrl } })); setError(''); }}>Load</Button></div></div></section>
    {source && <section className="grid gap-4 rounded-lg border p-4 lg:grid-cols-[minmax(0,1fr)_240px]"><div className="relative h-[480px] overflow-hidden rounded bg-muted"><Cropper image={source} crop={crop} zoom={zoom} rotation={rotation} aspect={PORTRAIT_CONFIG.width / PORTRAIT_CONFIG.height} onCropChange={setCrop} onZoomChange={setZoom} onRotationChange={setRotation} onCropComplete={(_, pixels) => setArea(pixels)} /></div><div className="space-y-4"><div><label className="text-sm font-medium">Zoom</label><Input type="range" min="1" max="3" step="0.01" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></div><div><label className="text-sm font-medium">Rotation</label><Input type="range" min="-180" max="180" step="1" value={rotation} onChange={(event) => setRotation(Number(event.target.value))} /></div><p className="text-xs text-muted-foreground">Drag the image to position it. Touch gestures and mouse-wheel zoom are supported.</p><Button type="button" onClick={save}>Crop and save portrait</Button><Button type="button" variant="outline" onClick={() => setDraft((current) => ({ ...current, utilities: { ...current.utilities, portraitDataUrl: '', portraitSourceDataUrl: '' } }))}>Clear portrait</Button></div></section>}
    ...
    {error && <p className="text-sm text-[#990000]">{error}</p>}
  </div>;
}

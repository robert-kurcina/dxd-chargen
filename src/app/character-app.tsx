...
export default function CharacterApp({ data }: { data: StaticData }) {
  ...
  return <main className={cn(tab === 'sheet' ? 'h-dvh overflow-hidden' : 'min-h-screen', 'p-2 md:p-8')}><Tabs value={tab} onValueChange={changeTab} className={cn('w-full', tab === 'sheet' && 'flex h-full min-h-0 flex-col')}><div data-forge-modal-background className="sticky top-0 z-50 shrink-0 bg-white py-2 print:hidden"><TabsList className="mx-auto grid w-full max-w-[1080px] grid-cols-5"><TabsTrigger value="forge">Forge</TabsTrigger><TabsTrigger value="sheet">Sheet</TabsTrigger><TabsTrigger value="library">Library</TabsTrigger><TabsTrigger value="tests">Tests</TabsTrigger><TabsTrigger value="info">Info</TabsTrigger></TabsList></div><div className={cn('pt-2', tab === 'sheet' && 'min-h-0 flex-1')}><TabsContent value="forge"><div data-forge-modal-background className="mx-auto mb-3 flex max-w-[1440px] flex-wrap items-center justify-between gap-2 rounded-lg border bg-card p-3"><div className="min-w-0 text-sm"><div className="truncate font-medium">{activeFileId ?? 'Unsaved character'}{dirty && <span className="ml-2 text-xs text-[#990000]">Unsaved changes</span>}</div>{message && <div className="text-xs text-muted-foreground" role="status" aria-live="polite">{message}</div>}</div><div className="ml-auto flex gap-2"><Button variant="outline" disabled={!activeFileId || !dirty} onClick={() => void revert()}>Revert</Button><Button disabled={!dirty} onClick={() => void save()}>Save</Button></div></div><Worksheet data={data} draft={draft} setDraft={setDraft} onReset={reset} /></TabsContent><TabsContent value="sheet" className="h-full min-h-0"><div className="mx-auto h-full min-h-0 w-full max-w-[1200px]"><ExpandedCharacterSheet draft={draft} data={data} /></div></TabsContent><TabsContent value="library"><CharacterLibraryPanel data={data} refreshKey={libraryRefresh} onOpen={loadDraft} /></TabsContent><TabsContent value="tests"><Tests data={data} /></TabsContent><TabsContent value="info"><Info data={data} /></TabsContent></div></Tabs></main>;
}

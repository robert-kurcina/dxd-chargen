...
function strifeSummary(draft: CharacterDraft, data: StaticData) {
  ...
  if (!draft.intrinsics.childOfStrife || !parents || (!pairing && !draft.intrinsics.strifeMixedLineage)) return null;
  const lineage = (group: string, id: string | null) => data.species.flatMap((family) => family.groups).find((item) => item.name === group)?.lineages.find((name) => `lineage-${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}` === id) ?? 'unassigned';
  const identity = draft.intrinsics.strifeMixedLineage ? `Mixed Lineage ${parents.fatherGroup}` : `${pairing?.exonym} (${pairing?.meaning})`;
...
function NotesEditor({ draft, data, setDraft }: { draft: CharacterDraft; data: StaticData; setDraft: Dispatch<SetStateAction<CharacterDraft>> }) {
  const generated = [
    `Personality: ${draft.background.personality.map((item) => item.name).join(', ') || '—'}`,
    `Blemishes: ${draft.background.demographicSelections.filter((item) => /blemish/i.test(item.sourceDetail ?? '')).map((item) => item.name).join(', ') || '—'}`,
    `Notable features: ${draft.background.demographicSelections.filter((item) => /notable/i.test(item.sourceDetail ?? '')).map((item) => item.name).join(', ') || '—'}`,
    `Tragedy seed: ${draft.background.tragedySeedText ?? '—'}`,
    strifeSummary(draft, data),
  ].filter(Boolean) as string[];
  const update = (field: 'notes' | 'backstory', value: string) => setDraft((current) => ({ ...current, utilities: { ...current.utilities, [field]: purifyNotes(value) } }));
  const names = (items: Array<{ name: string }>, separator = ', ') => [...items].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })).map((item) => item.name).join(separator) || '—';
  return <div className="grid gap-4"><section className="rounded-lg border p-4">{generated.map((line) => <div key={line} className="text-sm">{line}</div>)}</section><section className="rounded-lg border p-4"><div className="font-medium">Possessions and Spells</div><dl className="mt-3 grid grid-cols-[100px_1fr] gap-x-3 gap-y-2 text-sm"><dt className="text-muted-foreground">Weapons</dt><dd>{names(draft.utilities.weapons, '; ')}</dd><dt className="text-muted-foreground">Armor</dt><dd>{names(draft.utilities.armor, '; ')}</dd><dt className="text-muted-foreground">Equipment</dt><dd>{names(draft.utilities.equipment, '; ')}</dd><dt className="text-muted-foreground">Spells</dt><dd>{names(draft.utilities.spells)}</dd></dl></section><section className="space-y-2 rounded-lg border p-4"><div className="flex justify-between"><label className="font-medium">Notes</label><span className="text-xs text-muted-foreground">{formatNumberWithCommas(draft.utilities.notes.length)}/1,000</span></div><Textarea maxLength={1000} value={draft.utilities.notes} onChange={(event) => update('notes', event.target.value)} className="min-h-40" /></section><section className="space-y-2 rounded-lg border p-4"><div className="flex justify-between"><label className="font-medium">Backstory</label><span className="text-xs text-muted-foreground">{formatNumberWithCommas(draft.utilities.backstory.length)}/1,000</span></div><Textarea maxLength={1000} value={draft.utilities.backstory} onChange={(event) => update('backstory', event.target.value)} className="min-h-40" /></section></div>;
}
...
function keyDrivers(stepValue: string, draft: CharacterDraft, data: StaticData): string[] {
  ...
  const heritage = [draft.background.culturalHeritageId, draft.background.environHeritageId, draft.background.societalHeritageId]
    .map((id) => data.heritagePackages.find((pkg) => pkg.id === id)?.name)
    .filter(Boolean)
    .join(' / ');
  const attributeState = draft.intrinsics.attributes.length ? 'assigned' : 'unassigned';
  const kno = getFinalAttributeValue('KNO', draft);
  const budget = skillpointBudget(draft, data);

  switch (stepValue) {
    case 'background-demographics': return [`Species age brackets: ${species?.group.name ?? 'Species unassigned'}`];
    case 'background-heritage': return [`Region: ${region ? `${region.region} — ${region.name}` : 'unassigned'}`, `Settlement: ${settlement ?? 'unassigned'}`];
    case 'background-social-rank': return [`Society Heritage: ${data.heritagePackages.find((pkg) => pkg.id === draft.background.societalHeritageId)?.name ?? 'unassigned'}`];
    case 'background-disabilities': return [`Age Group: ${draft.background.ageGroup ?? 'unassigned'}`, `PML: ${draft.proficiencies.pml ?? 'unassigned'}`];
    case 'background-belief-worship': return [`Belief: ${data.beliefs.find((item) => item.catalogId === draft.background.beliefId)?.keyword ?? 'unassigned'} (controls deity requirement)`];
    case 'intrinsics-species': return ['Playable scope: Humaniki; Cherigili, Kriket, and Stonefolk unavailable'];
    case 'intrinsics-attributes': return [`Biology: ${[species?.group.name, lineage].filter(Boolean).join(' / ') || 'unassigned'}`, `Age Group: ${draft.background.ageGroup ?? 'unassigned'}`, 'Creation budget: 75 point-buy; +4 purchased increases'];
    case 'intrinsics-trade-specialization': return [`Age Group: ${draft.background.ageGroup ?? 'unassigned'}`, `Species / Lineage: ${[species?.group.name, lineage].filter(Boolean).join(' / ') || 'unassigned'}`, `Attributes: ${attributeState}`];
    case 'intrinsics-zed': return [`Critical Attribute Rolls: ${attributeState}`, `Trade: ${trade?.trade ?? 'unassigned'}`];
    case 'intrinsics-wealth': return [`Heritage: ${heritage || 'unassigned'}`, `Settlement: ${settlement ?? 'unassigned'}`, `KNO: ${kno ?? 'unassigned'}`];
    case 'proficiencies-pml': return [`Age Group: ${draft.background.ageGroup ?? 'unassigned'} (sets minimum age)`];
    case 'proficiencies-skills-abilities-talents': return [`Granted packages: ${[heritage && 'Heritage', species && 'Species', trade && 'Trade'].filter(Boolean).join(', ') || 'none'}`, `PML: ${draft.proficiencies.pml ?? 'unassigned'}`];
    case 'proficiencies-additional-skills': return [`Skillpoints: ${budget.remaining ?? 'unresolved'} remaining`, `Age: ${draft.background.ageYears ?? 'unresolved'}`, `Trade: ${trade?.trade ?? 'unassigned'}`];
    case 'proficiencies-languages': return [`Settlement: ${settlement ?? 'unassigned'}`, `Heritage: ${heritage || 'unassigned'}`, `INT / KNO: ${getFinalAttributeValue('INT', draft) ?? '—'} / ${kno ?? '—'}`];
    case 'properties-height-weight': return [`Species / Lineage: ${[species?.group.name, lineage].filter(Boolean).join(' / ') || 'unassigned'}`, `Age: ${draft.background.ageYears ?? 'unresolved'}`, `Heritage / Trade: ${heritage || 'unassigned'} / ${trade?.trade ?? 'unassigned'}`];
    case 'properties-calculations': return [`Attributes: ${attributeState}`, `Physical profile: ${draft.properties.profile ?? 'unresolved'}`, `PML: ${draft.proficiencies.pml ?? 'unassigned'}`];
    case 'utilities-spells': return [`v-Magic access: ${draft.proficiencies.granted.some((item) => item.name.replace(/\s+X$/, '') === 'v-Magic') ? 'present' : 'not present'}`];
    case 'utilities-starting-gear': return [`Personal Wealth: ${personalWealthGp(draft, data)?.toLocaleString() ?? 'unresolved'} gp`];
    case 'utilities-magic-items': return ['Canonical entitlement or GM approval; only complete records are available'];
    case 'utilities-name': return [`Naming language: ${data.languages.find((item) => item.id === draft.utilities.nameLanguageId)?.name ?? 'suggested from origin / known languages'}`];
    default: return [];
  }
}
...
export default function Worksheet({
  ...
  const stepAssessment = (stepValue: string) => {
    ...
    if (draft.completedSteps.includes(stepValue)) return { status: 'complete' as const, messages: [] as string[] };
    if (stepValue === 'notes-overview' || stepValue === 'notes-portrait') return { status: 'complete' as const, messages: [] as string[] };
  ...
  const activeKeyDrivers = activeStep ? keyDrivers(activeStep.value, draft, data) : [];
  ...
  const panelLanguages = [...panelDraft.proficiencies.languages]
    .sort((left, right) => left.name.localeCompare(right.name, undefined, { numeric: true, sensitivity: 'base' }));
  const panelInventory = [...panelDraft.utilities.weapons, ...panelDraft.utilities.armor, ...panelDraft.utilities.equipment]
    .sort((left, right) => left.name.localeCompare(right.name, undefined, { numeric: true, sensitivity: 'base' }));
  ...
  const alertCount = unresolvedSteps.filter(({ assessment }) => assessment.status === 'incomplete').length;
  const warningCount = unresolvedSteps.filter(({ assessment }) => assessment.status === 'warning').length;
  const speciesChoice = getSpeciesChoice(panelDraft, data);
  const importedDetail = (detail: string) => panelDraft.background.demographicSelections.find((entry) => entry.sourceDetail === detail)?.name;
  const selectedBelief = data.beliefs.find((item) => item.catalogId === panelDraft.background.beliefId);
  const beliefDisplay = selectedBelief?.isDeity ? (data.deities.find((item) => item.catalogId === panelDraft.background.deityId)?.deity ?? importedDetail('Imported religion detail') ?? 'Theist — Deity required') : ([selectedBelief?.keyword, importedDetail('Imported religion detail')].filter(Boolean).join(' / ') || '—');
  const calc = (key: string) => {
    const aliases: Record<string, string[]> = {
      Hitpoints: ['hitpoints'], Bodypoints: ['bodypoints'], Recovery: ['recoveryRate'],
      Resilience: ['resilience'], Resistance: ['resistance'], Endurance: ['endurance'],
    ...
    return calculated[key] ?? aliases[key]?.map((alias) => calculated[alias]).find((value) => value != null) ?? '—';
  };
  const attributePanelValue = (name: string) => {
    const final = getFinalAttributeValue(name, panelDraft);
    if (final != null) return final;
    const adjustment = nonPlayerAdjustmentsForAttribute(name, panelDraft, data)
      ...
      <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)_300px]">
        ...
        <Card className="min-h-[560px]">
          <CardHeader>
            ...
            {activeKeyDrivers.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 pt-2 text-xs">
                <span className="font-semibold uppercase tracking-wide text-muted-foreground">Key drivers</span>
                {activeKeyDrivers.map((driver) => <Badge key={driver} variant="outline" className="font-normal">{driver}</Badge>)}
              </div>
            )}
          ...
          <CardContent className="space-y-6">
            ...
            {activeStep.value === 'notes-overview' ? <NotesEditor draft={draft} data={data} setDraft={setDraft} /> : activeStep.value === 'notes-portrait' ? <PortraitStep draft={draft} setDraft={setDraft} /> : isBackgroundStep(activeStep.value) || isIntrinsicStep(activeStep.value) || isProficiencyStep(activeStep.value) || isPropertyStep(activeStep.value) || isUtilityStep(activeStep.value) ? (
              <>
                ...
                {activeAssessment.messages.length > 0 && (
                  <div className={cn(
                    ...
                    activeAssessment.status === 'warning' && 'border-yellow-400 bg-yellow-100 text-black',
                    activeAssessment.status === 'incomplete' && 'border-[#990000] text-[#990000]',
                  )}>
                    <div className="flex items-center gap-2 font-medium">
                      {activeAssessment.status === 'warning' && <AlertTriangle className="h-4 w-4 text-black" />}
                      {activeAssessment.status === 'incomplete' && <AlertTriangle className="h-4 w-4 text-[#990000]" />}
        ...
        <Card className="h-fit w-full lg:sticky lg:top-44">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <PersistentAccordionSection id="character-background" title="Background" defaultOpen={false}><dl className="grid grid-cols-[110px_1fr] gap-x-3 gap-y-2"><dt className="text-muted-foreground">Sex / Gender</dt><dd>{[panelDraft.background.sex, panelDraft.background.gender].filter(Boolean).join(' / ') || '—'}</dd><dt className="text-muted-foreground">Handedness</dt><dd>{panelDraft.background.handedness ?? '—'}</dd><dt className="text-muted-foreground">Birth Month</dt><dd>{panelDraft.background.birthMonth ?? '—'}</dd><dt className="text-muted-foreground">Personality</dt><dd>{panelDraft.background.personality.map((item) => item.name).join(', ') || '—'}</dd><dt className="text-muted-foreground">Tragedy</dt><dd>{panelDraft.background.tragedySeedText ?? '—'}</dd><dt className="text-muted-foreground">Disabilities</dt><dd>{panelDraft.background.disabilities.map((item) => item.name).join(', ') || (panelDraft.background.disabilitiesReviewed ? 'None' : '—')}</dd></dl></PersistentAccordionSection>
            <PersistentAccordionSection id="character-attributes" title="Attributes"><div className="space-y-3"><div><div className="mb-1 text-xs font-semibold text-muted-foreground">Combat</div><div className="grid grid-cols-3 gap-2">{['CCA','RCA','REF'].map((name) => <div key={name} className="rounded border p-2 text-center"><div className="text-xs text-muted-foreground">{name}</div><div className="font-semibold">{attributePanelValue(name)}</div></div>)}</div></div><div><div className="mb-1 text-xs font-semibold text-muted-foreground">Psychological</div><div className="grid grid-cols-4 gap-2">{['INT','KNO','PRE','POW'].map((name) => <div key={name} className="rounded border p-2 text-center"><div className="text-xs text-muted-foreground">{name}</div><div className="font-semibold">{attributePanelValue(name)}</div></div>)}</div></div><div><div className="mb-1 text-xs font-semibold text-muted-foreground">Physical</div><div className="grid grid-cols-3 gap-2">{['STR','FOR','MOV'].map((name) => <div key={name} className="rounded border p-2 text-center"><div className="text-xs text-muted-foreground">{name}</div><div className="font-semibold">{name === 'MOV' && calc('MOV') !== '—' ? calc('MOV') : attributePanelValue(name)}</div></div>)}</div></div><div><div className="mb-1 text-xs font-semibold text-muted-foreground">Magic</div><div className="w-24 rounded border p-2 text-center"><div className="text-xs text-muted-foreground">ZED</div><div className="font-semibold">{panelDraft.intrinsics.zed ?? attributePanelValue('ZED')}</div></div></div></div></PersistentAccordionSection>
            ...
            <PersistentAccordionSection id="character-misc" title="Miscellaneous"><dl className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-2">{[['PML',panelDraft.proficiencies.pml],['Affinity',panelDraft.intrinsics.affinityAttribute],['Wealth Rank',panelDraft.intrinsics.wealthRank ?? calc('wealthRank')],['Social Rank',panelDraft.background.socialRank ?? calc('socialRank')],['Trade Rank',panelDraft.intrinsics.tradeRank],['Favor Dice',calc('FavorDice')],['Cellburn Limit',calc('Cellburn')],['Manapool',calc('Manapool')]].map(([name,value]) => <Fragment key={String(name)}><dt className="text-muted-foreground">{name}</dt><dd>{value ?? '—'}</dd></Fragment>)}</dl></PersistentAccordionSection>
            <PersistentAccordionSection id="character-languages" title={`Languages (${panelLanguages.length})`} defaultOpen={false}><div className="space-y-1 text-xs">{panelLanguages.length ? panelLanguages.map((item) => <div key={item.id}>{item.name}{item.level != null ? ` ${item.level}` : ''}</div>) : '—'}</div></PersistentAccordionSection>

...
const cleanHistoryText = (value) => String(value ?? "").replace(/\r/g, "").trim();
const alphanumericKey = (value) => value.replace(/^[^a-z0-9]+/i, "");
const sortTextAscending = (values) => [...values].sort((left, right) => alphanumericKey(left).localeCompare(alphanumericKey(right), "en", {
    numeric: true,
    sensitivity: "base",
}));
function splitHistoryList(value) {
    ...
    for (let index = 0; index < source.length; index += 1) {
        ...
        if (braces === 0 && brackets === 0 &&
            ...
            if (item && !/^none(?: revealed)?$/i.test(item))
                output.push(item);
            token = "";
        }
        else {
...
function canonicalRankedTerm(value) {
    const text = cleanHistoryText(value);
...
function canonicalSpecializations(value) {
    const text = cleanHistoryText(value);
...
function canonicalZedsurge(value) {
...
export function parseHistoryNotes(value) {
    const source = cleanHistoryText(value);
    const sections = Object.fromEntries(HISTORY_NOTE_KEYWORDS.map((keyword) => [keyword, []]));
    ...
    sections.Skills.forEach((entry) => {
        ...
        if (virtuosity)
            traits.push(...virtuosity);
        else
            skills.push(canonicalSpecializations(entry));
    });
...
export function serializeHistoryNotes(sections) {
...
function splitEquipmentBlocks(value) {
    return cleanHistoryText(value)
        .split(/\n\s*\n+/)
        .map(cleanHistoryText)
        .filter(Boolean);
}
export function parseEquipmentRows(itemValue, propertyValue) {
    ...
    return rows.sort((left, right) => alphanumericKey(left.item).localeCompare(alphanumericKey(right.item), "en", {
        numeric: true,
        sensitivity: "base",
    }));
}
export function serializeEquipmentRows(rows) {
...
const TOOLTIP_ENDURANCE = "Physical Endurance. Ignore the effects of Fatigue and Weariness.";
const TOOLTIP_RESILIENCE = "Mental Resilience. Ignore the effects of Stress.";
const TOOLTIP_RESISTANCE = "Radiation Resistance. Ignore the effects of Rads.";
const TOOLTIP_FATIGUE = "Physical Fatigue. Acquire this when doing too much too quickly.";
const TOOLTIP_WEARINESS = "Physical Weariness. Acquire this when doing too much for too long.";
const TOOLTIP_STRESS = "Mental Stress. Acquire this when mental, psychological, or emotional health is attacked.";
const TOOLTIP_RADS = "Radiation Damage. The Wasting Sickness. Acquire this around too much magic.";
...
const SCALAR_UC = [1, 1.2, 1.5, 2, 2.5, 3, 4, 5, 6, 8];
const SCALAR_AF = {
...
function overrideOr(value, fallback) {
    return value === null || value === undefined || value === ""
        ? fallback
        : numberValue(value);
}
export function getDMByValue(value, usePrefix = false) {
...
export function getScalarValue(index) {
...
export function getIndex(value) {
    ...
    if (numeric >= 100000)
        return 40;
    const dekaGroup = numeric >= 10000 ? 30 : numeric >= 1000 ? 20
    ...
    SCALAR_UC.forEach((scalar, index) => {
        if (processValue >= scalar)
            offset = index;
    });
    return dekaGroup + offset;
}
export function getDegreeOfFear(stressDM) {
    if (stressDM <= 0)
        return "";
    if (stressDM <= 1)
        return "NERVOUS";
...
export function getShowDM(dm, actualValue) {
    if (actualValue === 0 || dm === 0)
        return "";
    return `>> suffer DM ${dm}D`;
}
export function countLevelStars(decals) {
    return decals.filter(({ name }) => name.toLowerCase() === "star").length;
}
export function getDeityRating(source) {
...
export function calculateCharacterSheet(source, decals = []) {
    const values = { ...source };
    const display = {};
    const get = (field) => numberValue(values[field]);
    const set = (field, value, tooltip) => {
        values[field] = value;
    ...
    set("IndexLift", lift);
    set("ScalarLift", getScalarValue(lift));
    set("IndexShoulder", shoulder);
    set("ScalarShoulder", getScalarValue(shoulder));
    set("IndexCarry", carry);
    set("ScalarCarry", getScalarValue(carry));
    ...
    set("IndexLob", lob);
    set("ScalarLob", getScalarValue(lob));
    set("IndexPitch", pitch);
    set("ScalarPitch", getScalarValue(pitch));
    set("IndexHurl", hurl);
    set("ScalarHurl", getScalarValue(hurl));
    const movementAdjustment = Math.floor(numberValue(getDMByValue(reflex)) / 2 + numberValue(getDMByValue(strength)) / 2
    ...
    set("IndexWalk", walk);
    set("ScalarWalk", getScalarValue(walk));
    set("IndexJog", jog);
    set("ScalarJog", getScalarValue(jog));
    set("IndexRun", run);
    set("ScalarRun", getScalarValue(run));
    ...
    set("IndexUp", jumpUp);
    set("ScalarUp", getScalarValue(jumpUp));
    set("IndexBroad", jumpBroad);
    ...
    const concern = (field, capacity, tooltip) => {
...
export function serializeCharacterSheet(values) {
    const textbox = Object.fromEntries(SAVED_TEXT_FIELDS.map((field) => [field, values[field] ?? ""]));
    return JSON.stringify({ checkbox: {}, counter: {}, textbox });
}
export function deserializeCharacterSheet(serialized) {
    if (serialized.trim() === "")
        return {};
